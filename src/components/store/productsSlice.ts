...
export async function fechData(dispatch: Dispatch<AnyAction>, id: string | undefined, page: number,limit: string) {
    ...
    try {
        if (id === "0" || id === null) {
            const response = await axios.get(`https://shop-api-backend-main.vercel.app/api/products/allinfo`);
...
export async function fechSingleProduct(dispatch: Dispatch<AnyAction>, id: string | string[] | undefined) {
    ...
    try {
        const response = await axios.get(`https://shop-api-backend-main.vercel.app/api/products/info/${id}`);
